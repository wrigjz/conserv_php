...
###################################################################################################
# This php ver. 7 script takes a given file and uploads it and checks that it appears to be a valid FASTA file
...
$input_file = $target_dir . "input.fasta";
$uploadOk = 1;
$FASTAFileType = strtolower(pathinfo($target_file, PATHINFO_EXTENSION));

// Check if fasta file is a reasonable size or not
if (isset($_POST["submit"])) {
    $check = filesize($_FILES["fileToUpload"]["tmp_name"]);
    if ($check !== false) {
        echo "File is a fasta file - " . $check["mime"] . ".";
        $uploadOk = 1;
    } else {
        echo "File is not a fasta file. ";
        $uploadOk = 0;
    }
}
